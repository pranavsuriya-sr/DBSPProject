...
connector.execute(
    "CREATE TABLE IF NOT EXISTS SCHOOL_MANAGEMENT (STUDENT_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, NAME TEXT, EMAIL TEXT, ROLL_NO TEXT, GENDER TEXT, DOB TEXT, STREAM TEXT, MARKS TEXT, PASSWORD TEXT)"
)

# Creating the functions
def reset_fields():
    global name_strvar, email_strvar, roll_strvar, gender_strvar, dob, stream_strvar
    for i in ['name_strvar', 'email_strvar', 'roll_strvar', 'gender_strvar', 'stream_strvar']:
        exec(f"{i}.set('')")
...
def add_record():
    ...
    if not name or not email or not roll or not gender or not DOB or not stream:
        mb.showerror('Error!', "Please fill all the missing fields!!")
    else:
        try:
            connector.execute(
                'INSERT INTO SCHOOL_MANAGEMENT (NAME, EMAIL, ROLL_NO, GENDER, DOB, STREAM, MARKS, PASSWORD) VALUES (?,?,?,?,?,?,NULL, NULL)',
                (name, email, roll, gender, DOB, stream)
            )
            connector.commit()
...
Label(left_frame, text="roll Number", font=labelfont, bg=lf_bg).place(relx=0.175, rely=0.18)
...
Entry(left_frame, width=19, textvariable=roll_strvar, font=entryfont).place(x=20, rely=0.23)
...
tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                   columns=('Student ID', "Name", "Email Address", "Roll Number", "Gender", "Date of Birth", "Stream", "Marks", "Password"))
